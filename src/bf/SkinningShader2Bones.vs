...
// VERTEX_NORMAL is in mesh space and the bone matrices are deltas from the bind pose, so
// the normal is skinned exactly like the position above. Without this the normal keeps
// pointing the way it did when the mesh was authored, which makes the lit side of an
// animated mesh follow the geometry instead of the sun.
// r3.x and r3.y still hold the constant offsets of the two bones.
mov a0.x, r3.x
...
dp3 r4.y, VERTEX_NORMAL, c[a0.x+BONE_START_2]
dp3 r4.z, VERTEX_NORMAL, c[a0.x+BONE_START_3]
mad r5.xyz, r4.xyz, WEIGHTS.y, r5.xyz

// Blending two rotated normals shortens the result across a joint, so renormalize.
